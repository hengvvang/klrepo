我理解你的需求，你希望内容更有条理、更详细，且专注于 STM32F103C8T6 标准库中每个外设的结构体、数据类型和函数，突出细节并总结规律。以下是重新优化后的输出，我将按统一格式逐一介绍所有外设，确保内容清晰、详尽且有逻辑性。每个外设将包括功能简介、硬件寄存器结构体、初始化结构体、关键枚举类型、API 函数列表及其参数说明，最后总结规律。
